...
public class ApiResultFunc<T> implements Function<ResponseBody, ApiResult<T>> {
            ...
            if (ApiResult.class.isAssignableFrom(cls)) {
                ...
                try {
                    ...
                    if (!List.class.isAssignableFrom(rawType) && clazz.equals(String.class)) {
                        apiResult = parseApiResult(json, apiResult);
                    } else {
                        apiResult = parseApiResult(json, apiResult);
                        apiResult.setData((T) JSON.parseObject(json, rawType));
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                    apiResult.setMsg(e.getMessage());
                    //  apiResult.setMsg(responseBody.string());
                } finally {
                    responseBody.close();
                }
            } else {
    ...
    private ApiResult parseApiResult(String json, ApiResult apiResult) throws JSONException {
        ...
        if (jsonObject.has("errorCode")) {
            apiResult.setCode(jsonObject.getString("errorCode"));
        }
    ...
    //  ApiResult{code='0', msg='null', data={"message":"服务成功返回请求数据","errorCode":"200","data":"eyJhbGciOiJIUzI1NiJ9.eyJpc3MiOiLmlrDnm5vok50i
    //  LCJwbGF0Zm9ybUlkIjoxLCJleHAiOjE1MzU5NzIwNDYsInVzZXJJZCI6MSwiaWF0IjoxNTM1OTY4NDQ2fQ.RrPuautEVMNKgSB6U776arZNf47hDM9v0S7qHiUW2B0","success":true}}
    /*private ApiResult parseApiResult(String json, ApiResult apiResult) throws JSONException {
        if (TextUtils.isEmpty(json))
            return null;
        JSONObject jsonObject = new JSONObject(json);
