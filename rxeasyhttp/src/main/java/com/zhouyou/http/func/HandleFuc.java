...
public class HandleFuc<T> implements Function<ApiResult<T>, T> {
    ...
    public T apply(@NonNull ApiResult<T> tApiResult) throws Exception {
        if (ApiException.isOk(tApiResult)) {
            return tApiResult.getData() == null ? (T) "" : tApiResult.getData();// == null ? Optional.ofNullable(tApiResult.getData()).orElse(null) : tApiResult.getData();
        } else {
            throw new ServerException(Integer.parseInt(tApiResult.getCode()), tApiResult.getMsg());
        }
    }
}
